...
class ReturningPage(QWidget):
    ...
    def _create_scan_page(self):
        """Page for scanning RFID tags of products to return"""
        page = QWidget()
        ...
        # Title
        title = QLabel("Scan Products to Return")
    ...
    def _create_login_page(self):
        ...
        # Title
        title = QLabel("User Login for Return")
        title.setStyleSheet("font-size: 18px; font-weight: bold;")
        layout.addWidget(title, alignment=Qt.AlignmentFlag.AlignCenter)
        ...
        btn_submit.clicked.connect(self.process_return)
    ...
    def _create_confirm_page(self):
        """Confirmation page for returning"""
        page = QWidget()
        layout = QVBoxLayout(page)
        ...
        self.return_table.setColumnCount(3)
        self.return_table.setHorizontalHeaderLabels(["Item", "Status", "RFID EPC"])
        self.return_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        content_layout.addWidget(self.return_table)
        
        # Return button
        btn_return = QPushButton("Confirm Return")
        btn_return.clicked.connect(self.confirm_return)
        content_layout.addWidget(btn_return)
    ...
    def _fetch_asset_details(self, epc: str, uid: str):
        """Fetch asset details from API and check if borrowed"""
        try:
            progress = QProgressDialog("Checking product status...", None, 0, 0, self)
            ...
            if response.status_code == 200:
                ...
                if isinstance(assets, list) and len(assets) > 0:
                    ...
                    if matched_asset:
                        ...
                        if matched_asset.get('status', '').lower() != 'borrowed':
                            QMessageBox.warning(self, "Not Borrowed", 
                                f"Product {matched_asset.get('name')} is not currently borrowed")
                            progress.close()
                            return
            ...
            if response.status_code == 200:
                ...
                if isinstance(assets, list) and len(assets) > 0:
                    asset = assets[0]
                    # Check if asset is borrowed
                    if asset.get('status', '').lower() != 'borrowed':
                        QMessageBox.warning(self, "Not Borrowed", 
                            f"Product {asset.get('name')} is not currently borrowed")
                        return
    ...
    def _add_asset_to_table(self, asset_data: dict):
        """Add borrowed asset to the table"""
        try:
    ...
    def proceed_to_login(self):
        """Move to login page for return"""
        self.stack.setCurrentWidget(self.login_page)

    def process_return(self):
        """Process user login and prepare return"""
        ...
        try:
            ...
            self.user_data = {
                ...
                'userId': login_data.get('userId')  # We'll need this to verify ownership
            }
    ...
    def _prepare_confirmation_page(self):
        ...
        for asset in self.scanned_assets:
            ...
            self.return_table.setItem(row, 0, QTableWidgetItem(asset.get('name', 'N/A')))
            self.return_table.setItem(row, 1, QTableWidgetItem(asset.get('status', 'N/A')))
            self.return_table.setItem(row, 2, QTableWidgetItem(rfid_tag.get('epc', 'N/A')))

    def confirm_return(self):
        """Finalize the return process"""
        if not self.token or not self.scanned_assets:
            QMessageBox.warning(self, "Warning", "Invalid return data")
            return
            
        try:
            progress = QProgressDialog("Processing return...", None, 0, 0, self)
            progress.setWindowModality(Qt.WindowModality.WindowModal)
            progress.setCancelButton(None)
            progress.show()
            QApplication.processEvents()
            ...
            response = requests.post(
                'http://localhost:5000/api/borrowing/return',
                headers=headers,
                json=payload,
                timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                self._reset_return_flow()
            else:
                error_msg = response.json().get('message', 'Return failed')
                raise Exception(error_msg)
                
        except Exception as e:
            progress.close()
            QMessageBox.critical(self, "Error", f"Return failed: {str(e)}")

    def _reset_return_flow(self):
        """Reset the return process"""
        self.scanned_assets = []
    ...
    # RFID-related methods (similar to other pages)
